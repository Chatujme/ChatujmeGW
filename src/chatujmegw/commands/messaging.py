...
def _handle_ctcp(sess, command, target, text):
    ...
    if ctcp_cmd == "VERSION":
        ...
        sess.send_raw(f":{sess.user.me} NOTICE {sess.user.nick} :\x01VERSION {version_reply}\x01\r\n")
    elif ctcp_cmd == "PING":
        # Echo back PING for latency measurement
        ping_data = text.strip('\x01')[5:].strip()  # Get data after "PING "
        sess.send_raw(f":{sess.user.me} NOTICE {sess.user.nick} :\x01PING {ping_data}\x01\r\n")
    elif ctcp_cmd == "TIME":
        # Reply with current time
        time_str = time.strftime("%a %b %d %H:%M:%S %Y")
        sess.send_raw(f":{sess.user.me} NOTICE {sess.user.nick} :\x01TIME {time_str}\x01\r\n")
        ...
        if action_text and target.startswith('#'):
            ...
            room = sess.is_in_room(room_id, True)
            if room:
...
def privmsg(sess, parts, line):
    ...
    if not sess.user.login:
        sess.send(sess.rfc.ERR_NOLOGIN, ":You have not registered")
        return
    ...
    if len(text) > config.MAX_MESSAGE_LENGTH:
        ...
        sess.send_raw(f":{sess.user.me} NOTICE {sess.user.nick} :Message truncated to {config.MAX_MESSAGE_LENGTH} characters\r\n")
    ...
    if is_pm:
        if not sess.rooms:
            sess.send(sess.rfc.ERR_NOSUCHNICK, f"{target} :Cannot send PM - join a room first")
            return
        text = f"/m {target} {text}"
        room_id = sess.rooms[0].id
        sess.rooms[0].idler_lastsend = time.time()
    else:
        room_id = target.lstrip('#')
        room = sess.is_in_room(room_id, True)
        if room:
