...
def cap(sess, parts, line):
    ...
    if subcmd == "LS":
        ...
        sess.send_raw(f":{sess.server_name} CAP * LS :{caps_str}\r\n")
        sess.cap_negotiating = True
    elif subcmd == "LIST":
        # List enabled capabilities
        caps_str = ' '.join(SUPPORTED_CAPS)
        sess.send_raw(f":{sess.server_name} CAP * LIST :{caps_str}\r\n")
    elif subcmd == "REQ":
        ...
        if all_supported:
            sess.send_raw(f":{sess.server_name} CAP * ACK :{caps}\r\n")
        else:
            sess.send_raw(f":{sess.server_name} CAP * NAK :{caps}\r\n")
    elif subcmd == "END":
...
def nick(sess, parts, line):
    if len(parts) < 2:
        return
    if sess.user.login:
        sess.send_raw(f":{sess.server_name} NOTICE {sess.user.username} :Already logged in\r\n")
        return
...
def nickserv(sess, parts, line):
    ...
    if len(parts) > 1:
        ...
        if subcmd == "identify" or subcmd == "id":
            ...
            sess.send_raw(f":NickServ!services@{sess.server_name} NOTICE {sess.user.nick} :You are already identified.\r\n")
            ...
            sess.send_raw(f":NickServ!services@{sess.server_name} NOTICE {sess.user.nick} :Unknown NickServ command: {subcmd}\r\n")


def register(sess, parts, line):
    # Direct REGISTER command - redirect to web
    sess.send_raw(f":{sess.server_name} NOTICE * :Registration is only available via web.\r\n")
    sess.send_raw(f":{sess.server_name} NOTICE * :Please visit: https://chatujme.cz/registrace\r\n")
